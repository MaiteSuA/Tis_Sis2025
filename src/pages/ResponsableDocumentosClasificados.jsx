...
*/
  ...
  const extractExcelData = async () => {
    ...
    // Columnas esperadas en el Excel: ID_Inscrito (opcional), Competidor, Estado, Fase
    return json.map((r) => ({
      id_inscrito: r.ID_Inscrito ? Number(r.ID_Inscrito) : null,
      competidor: String(r.Competidor ?? "").trim(),
      estadoTexto: String(r.Estado ?? "").trim(),
      fase: Number(r.Fase ?? 2),
    }));
  };
